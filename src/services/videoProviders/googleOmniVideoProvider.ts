...
export class GoogleOmniVideoProvider extends BaseVideoProvider {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw {
          ...
          detail: errorData.detail || errorData.message || 'Terjadi kesalahan saat memanggil API Google Omni Video.',
          providerId: this.config.providerId,
  ...
  public async testConnection(): Promise<VideoProviderTestResult> {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        ...
        return {
          success: false,
          message: data.message || 'Koneksi ke server Google Omni 1.1 gagal.',
          latencyMs,
          providerId: this.config.providerId
        };
      }
      ...
      return {
        success: true,
        message: data.message || 'Koneksi ke Google Omni 1.1 aktif!',
