    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw {
          ...
          detail: errorData.detail || errorData.message || 'Terjadi kesalahan saat memanggil API Seedance Video.',
          providerId: this.config.providerId,
  ...
  public async testConnection(): Promise<VideoProviderTestResult> {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        ...
        return {
          success: false,
          message: data.message || 'Koneksi ke server Seedance gagal.',
          latencyMs,
          providerId: this.config.providerId
        };
      }
      ...
      return {
        success: true,
        message: data.message || 'Koneksi ke Seedance 2.0 Fast Pro aktif!',
