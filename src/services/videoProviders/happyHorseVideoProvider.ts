...
export class HappyHorseVideoProvider extends BaseVideoProvider {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw {
          ...
          detail: errorData.detail || errorData.message || 'Terjadi kesalahan saat memanggil API Happy Horse Video.',
          providerId: this.config.providerId,
  ...
  public async testConnection(): Promise<VideoProviderTestResult> {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        ...
        return {
          success: false,
          message: data.message || 'Koneksi ke server Happy Horse 1.0 gagal.',
          latencyMs,
          providerId: this.config.providerId
        };
      }
      ...
      return {
        success: true,
        message: data.message || 'Koneksi ke Happy Horse 1.0 aktif!',
